def _gather_info(keyword, sent, subjects, objects, times, locations, persons):
    '''
...
def _gather_info_batch(keyword, sents, subjects_all, objects_all, times, locations, persons):
    ...
    for i, sent in enumerate(sents):
        info = _gather_info(keyword, sent, subjects_all[i], objects_all[i], times[i], locations[i], persons[i])
